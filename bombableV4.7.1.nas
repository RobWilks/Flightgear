...
var ground_loop = func( id, myNodeName ) {
	...
	# rjw might always return if high in the sky? 
	...
	# rjw modified aircraftCrashControl no longer uses pitchAngle to force down
	...
	# rjw removed next section since changed aircraftCrashControl
...
targetSize_m = nil,  weaponSkill = 1, maxDistance_m = 100, weaponAngle_deg = nil, weaponOffset_m = nil, damageValue = 0 ) {
	...
	var missileSpeed_mps = 2000; # rjw muzzle velocity about 4000 knots
	...
	if (cosOffset > 0.5)
	{
		...
		#debprint ("Bombable: hit ", myNodeName1, " result.pHit = ", result.pHit);
	}
	...
	foreach (elem;keys (weaps) ) 
	{
		...
		# Could also check weaponSkill here. However skill of gunner not simply correlated with how frequently they fire
		{
			...
			# a skilled gunner will fire at 1% pHit; an unskilled one 0.1%
			{
				...
				fireAIWeapon(loopTime * .5, myNodeName1, elem, count, newAim.interceptSpd); # rjw shortened burst time from 3 to .5
				...
				if (r < newAim.pHit) {
					...
					var damageAdd = newAim.pHit * weaponPower; # rjw change to make impregnable main aircraft!
					# var damageAdd = newAim.pHit * weaponPower * 0; # rjw change to make impregnable main aircraft!
...
var weaponsOrientationPositionUpdate_loop = func (id, myNodeName) {
	...
	settimer (func {weaponsOrientationPositionUpdate_loop (id, myNodeName)}, .25);
...
# added since it is the main change in bombable made since the branch from the original code 
